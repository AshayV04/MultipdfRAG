...
def rewrite_summarize_question(question: str, doc_index: List[dict]) -> str:
    qlow = question.lower()
    if "summarize" not in qlow and "summary" not in qlow:
        return None
    for d in doc_index:
        title_low = d["title"].lower()
        if title_low in qlow:
            return f"Summarize the paper titled {d['title']}. Focus on its main idea, contribution, and problem it solves."
        short_title = title_low.split(":")[0]
        if short_title and short_title in qlow:
            return f"Summarize the paper titled {d['title']}. Focus on its main idea, contribution, and problem it solves."
    return None
...
def get_chunks_for_doc(question: str, vectorstore, target_doc_id: int, k: int = 4, first_page_k: int = 2):
    if not vectorstore:
        return []
    retrieved = vectorstore.similarity_search(question, k=20)
    filtered = []
    seen = set()
...
def get_top_chunks_for_doc(question, vectorstore, doc_id, k=3, first_page_k=1):
    if not vectorstore:
        return []
    retrieved = vectorstore.similarity_search(question, k=15)
    out = []
    seen = set()
    for doc in retrieved:
        meta = doc.metadata or {}
...
def handle_question(question):
    ...
    rewrite = rewrite_summarize_question(question, st.session_state.get("doc_index", [])) if summarize_hit else None
    if summarize_hit and rewrite:
        question = rewrite
    elif summarize_hit:
        answer = summarize_all_docs()
        st.write(
    ...
    target_doc = match_document_from_question(question, st.session_state.get("doc_index", []))
    if target_doc:
        rewritten = f"{question} Answer only using the paper titled {target_doc['title']}."
