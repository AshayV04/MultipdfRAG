...
def handle_question(question):
    ...
    target_doc_for_summary = match_document_from_question(question, st.session_state.get("doc_index", [])) if summarize_hit else None
    if summarize_hit and not target_doc_for_summary and qlow.strip() in ["summarize", "summary"]:
        answer_text, used_docs = build_multi_doc_summary(st.session_state.get("vectorstore"), st.session_state.get("doc_index", []))
    ...
    if target_doc:
        ...
        src_labels = sorted({display_label(d.metadata) for d in used_docs}) if used_docs else []
        src_note = "<br/><sub>Sources: " + ", ".join(src_labels) + "</sub>" if src_labels else ""
        render_chat(question, to_text(answer), src_labels)
        return
...
def render_chat(question, answer_text, src_labels=None):
